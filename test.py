...
class retirementSimulator():
    ...
    def __init__(self, starting_wealth=-30000, rate_of_return=0.07,
                 cost_of_living=40000, inflation=5.1,
                 wage=80000, yearly_raise=0.027,
                 withdrawl_rate=0.4, start_working_age=22,
                 target_retirement_age=64, work_till_at_least=50, death_age=94,
        ...
        # Doesn't really matter. This simulation isn't trying to use all our wealth by death anyways.
    ...
    def calculate_longterm_cap_gains_tax(self, amnt_to_sell, yrs_since_base):
        ...
        for bracket in adj_brackets:
            if amnt_to_sell > bracket[0]:
                tax += adj_brackets[bracket] * \
                    (min(amnt_to_sell, bracket[1])-bracket[0])
        return tax
    ...
    def calculate_federal_tax(self, yrs_since_base):
        ...
        for bracket in adj_brackets:
            if self.wage > bracket[0]:
                tax += adj_brackets[bracket] * \
                    (min(self.wage, bracket[1])-bracket[0])
        return tax
    ...
    def run_simulation(self):
        ...
        for i in range(self.start_working_age, self.target_retirement_age):
            ...
            withdrawl_post = withdrawl_pre - self.calculate_longterm_cap_gains_tax(
                withdrawl_pre, yrs_since_base=i - self.start_working_age)
            lifetime.append({'Age': i, 'Total Wealth': self.total_wealth,
                             ...
                             # PDV calculation
                             'Theoretical Surplus (Present $)': (withdrawl_post - self.cost_of_living)/(1+self.inflation)**(i - self.start_working_age)
                             })

            state_tax = self.calculate_state_tax()
            federal_tax = self.calculate_federal_tax(
                yrs_since_base=i - self.start_working_age)
            ...
            self.total_wealth += (self.wage - state_tax - federal_tax -
                                  self.cost_of_living)+self.total_wealth*self.rate_of_return
    ...
    def get_earliest_retirement(self):
        ...
        if self.summaryDf is not None:
            ...
            vec = (self.summaryDf['Withdrawl (post tax)'] > self.summaryDf['Cost of Living']) | (
                self.summaryDf['Theoretical Withdrawl (post tax)'] > self.summaryDf['Cost of Living'])
            ...
            if any(vec):
                ...
                new_vec = (newSimulation.summaryDf['Withdrawl (post tax)'] > newSimulation.summaryDf['Cost of Living']) | (
                    newSimulation.summaryDf['Theoretical Withdrawl (post tax)'] > newSimulation.summaryDf['Cost of Living'])
                runs_out = int(
                    newSimulation.summaryDf[new_vec].iloc[-1]['Age'])

                # runs_out = self.summaryDf[vec].iloc[-1]['Age']

                starting_retirement_wealth = round(
                    newSimulation.summaryDf[vec].iloc[0]['Total Wealth'], 2)
                ending_retirement_wealth = round(
                    newSimulation.summaryDf[vec].iloc[-1]['Total Wealth'], 2)

                delta_TW_retirement = round(
                    ending_retirement_wealth - starting_retirement_wealth, 2)
                increasing_TW = ending_retirement_wealth > starting_retirement_wealth

                if runs_out < self.death_age:
                    print("You can retire at ", best_age,
                          " but you will run out of money to pay for the cost of living at age ", runs_out, ".", sep="")
                else:
                ...
                print(
                    "You will not ever be able to meet your retirement goals! =(")
...
def main():
    ...
    starting_wealth = st.number_input('Starting Wealth:', value=-30000)
    rate_of_return = st.number_input('Rate of Return:', value=0.07)
    cost_of_living = st.number_input('Cost of Living:', value=40000)
    # Add more input widgets for other parameters

    # Create a button to run the simulation
    if st.button('Run Simulation'):
        # Create a RetirementSimulator instance with input parameters
        simulator = retirementSimulator(
            starting_wealth=starting_wealth, rate_of_return=rate_of_return, cost_of_living=cost_of_living)
        simulator.run_simulation()

        # Display simulation results
        st.write('Simulation Results:')
        st.dataframe(simulator.summaryDf)

        # Display earliest retirement age
        st.write('Earliest Retirement Age:')
        simulator.get_earliest_retirement()
